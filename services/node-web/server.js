...
// Configuration
const config = {
    goBase: process.env.GO_ISS_BASE || 'http://go_iss:3000',
...
app.get('/dashboard', async (req, res) => {
    try {
        // Fetch ISS data from go_iss - response format: { ok: true, data: { payload: {...} } }
        const issResponse = await fetchJson(`${config.goBase}/last`);
        // Extract payload from the nested data structure
        const data = issResponse.data || {};
...
app.get('/api/iss/last', async (req, res) => {
    try {
        const data = await fetchJson(`${config.goBase}/last`);
        res.json(data);
    } catch (error) {
...
app.get('/api/iss/trend', async (req, res) => {
    try {
        ...
        const url = `${config.goBase}/iss/trend${queryString ? '?' + queryString : ''}`;
 ...
 * Note: This is an optional feature. If credentials are not configured
 * or the API returns errors (e.g., 403), a fallback response is provided.
 */
app.get('/api/astro/events', async (req, res) => {
    try {
        ...
        // If credentials not configured, return empty with helpful message
        if (!config.astroAppId || !config.astroAppSecret) {
            console.log('AstronomyAPI: credentials not configured (ASTRO_APP_ID, ASTRO_APP_SECRET)');
            return res.json({ 
                ok: true, 
                message: 'AstronomyAPI not configured - this feature is optional',
                data: { events: [] } 
            });
        }
        ...
        // Handle 403 gracefully - this means credentials are invalid or expired
        if (error.response && error.response.status === 403) {
            console.log('AstronomyAPI: 403 Forbidden - credentials may be invalid or expired');
            return res.json({ 
                ok: true, 
                message: 'AstronomyAPI credentials invalid or expired - this feature is optional',
                data: { events: [] } 
            });
        }
        console.error('Astro events error:', error.message);
        res.json({ ok: true, message: error.message, data: { events: [] } });
    }
});
...
app.get('/osdr', async (req, res) => {
    try {
        const data = await fetchJson(`${config.goBase}/osdr/list`);
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`  Go API: ${config.goBase}`);
