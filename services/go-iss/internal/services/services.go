...
func (s *IssService) CalculateTrend(ctx context.Context) (*domain.IssTrend, error) {
	// Get more points for the chart (default 240)
	rows, err := s.repo.GetLastN(ctx, 240)
	if err != nil {
		return nil, err
	}

	if len(rows) < 2 {
		return &domain.IssTrend{Points: []domain.IssPoint{}}, nil
	}

	// Build points array for chart (in chronological order)
	points := make([]domain.IssPoint, 0, len(rows))
	for i := len(rows) - 1; i >= 0; i-- {
		row := rows[i]
		lat := getFloat(row.Payload, "latitude")
		lon := getFloat(row.Payload, "longitude")
		velocity := getFloat(row.Payload, "velocity")
		altitude := getFloat(row.Payload, "altitude")

		point := domain.IssPoint{
			At: row.FetchedAt,
		}
		if lat != nil {
			point.Lat = *lat
